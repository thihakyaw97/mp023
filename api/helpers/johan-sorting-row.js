module.exports = {
  ...
  friendlyName: 'Johan sorting row',
  ...
  inputs: {
    sort: {
      type: 'string',
      description: 'Sorting the Johan to show in notepad. rather than managing database asc and dsc, I prefer customizable sorting the scripts.'
    },
  },
  ...
  fn: async function (inputs,exits) {
    ...
    if(inputs.sort=="latest"){
      const sorting = await Johan.find();
    ...
    else if(await Johan.find().where({id:inputs.sort})){
      sorting = await Johan.find().where({id:inputs.sort});

      //GETTING THE SORTING NUMBERS
      sorting_update = await Johan.find().where({
        sort: {
            '>=':   sorting[0].sort
        }
      }).sort('sort ASC');
